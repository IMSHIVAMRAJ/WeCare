...
const BeauticianSlots = () => {
  ...
  const fetchBeauticianSlots = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/beauticianslot`, {
        headers: {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h2 className="text-2xl font-bold mb-6 text-center">Experts Availability</h2>
      ...
      {beauticians.map((beautician, index) => {
        ...
        return (
          <div key={index} className="bg-white shadow-md rounded-lg p-6 mb-6 border">
            <div className="mb-4">
              <h3 className="text-xl font-semibold text-purple-600">{beautician.name}</h3>
            ...
            {availableSlots.length > 0 ? (
              ...
              <p className="text-sm text-purple-500">No available slots</p>
            )}
          </div>
        );
      })}
