...
const Medical = () => {
  ...
  const fetchMakeupServices = async () => {
    try {
      ...
      const makeupServices = res.data.filter(service => service.category === "Medical");

      setAllServices(makeupServices);
  ...
  return (
    <div className="p-4 bg-gray-50 min-h-screen pb-20">
      <h1 className="text-2xl font-bold text-center mb-6 text-purple-600">
        Medical & Hygiene Support
      </h1>

      {/* Horizontal Subcategories - Oval Style */}
