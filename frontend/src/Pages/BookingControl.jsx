...
const AdminBookingControl = () => {
  ...
  const fetchBookings = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/allbookings`, {
  ...
  const fetchBeauticians = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/beauticians`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
      });
      setBeauticians(data);
    } catch (err) {
  ...
  const assignBeautician = async (bookingId, beauticianId) => {
    try {
      await axios.patch(
        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/bookings/${bookingId}/assign`,
        { beauticianId },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
        }
  ...
  return (
    <div className="p-6">
      ...
      <section className="mb-10">
        <h2 className="text-xl font-semibold mb-2 text-purple'-600">Pending Bookings (Needs Approval)</h2>
      ...
      {/* ✅ Approved Bookings */}
      <section>
        <h2 className="text-xl font-semibold mb-2 text-purple-600">Approved Bookings</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-left border">
            <thead>
              <tr className="bg-purple-100">
