...
const BeauticianBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const { data } = await axios.get('${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/beautician/my-bookings', {
          headers: {
            ...
            {filters.map(f => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                  activeFilter === f.key ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
              >
                {f.label} ({f.count})
              </button>
