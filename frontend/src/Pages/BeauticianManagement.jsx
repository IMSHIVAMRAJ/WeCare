...
const BeauticianManagement = () => {
  ...
  const fetchBeauticians = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/beauticians`, {
        headers: {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const res = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/beautician/add`, formData, {
        headers: {
  ...
  const handleRemove = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      await axios.delete(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/beautician/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div className="w-full p-4">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-bold text-black">Expert Management</h3>
        <button
          className="bg-gradient-to-r from-purple-500 to-purple-400 text-white px-5 py-2 rounded-lg font-semibold shadow hover:opacity-90 transition"
      ...
      <div className="overflow-x-auto rounded-xl shadow bg-white">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {beauticians.map((b) => (
              <tr key={b._id} className="border-b hover:bg-purple-50/40">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          ...
          >
            <button
              type="button"
              className="absolute top-2 right-4 text-gray-400 hover:text-purple-500 text-2xl"
              onClick={() => setShowModal(false)}
            >
              &times;
            </button>
            <h2 className="text-lg font-bold mb-4 text-black">Add Expert</h2>
            ...
            <input
              ...
              className="w-full mb-4 px-4 py-2 border border-purple-300 rounded-lg"
              requipurple
            />
