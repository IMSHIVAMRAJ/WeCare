...
const CouponManagement = () => {
  ...
  const fetchCoupons = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/coupons`, {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const res = await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/admin/coupon`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });
      alert("Coupon added ✅");
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          className="bg-gradient-to-r from-[#952ca8] to-[#7016a4] text-white px-5 py-2 rounded-lg font-semibold shadow hover:opacity-90 transition"
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {coupons.map((c) => {
              ...
              return (
                <tr key={c._id} className="border-b last:border-b-0 hover:bg-purple-50/40">
          ...
          >
            ...
            <div className="mb-4">
              ...
              <input
                ...
                required
                className="w-full px-4 py-2 rounded-lg border-2 border-purple-200 bg-purple-50 text-black"
              />
            </div>
            ...
            <button
              type="submit"
              className="w-full py-3 bg-gradient-to-r from-[#922fcb] to-[#4b0c66] text-white rounded-lg font-bold text-lg shadow-md"
            >
              Add Coupon
            </button>
          </form>
